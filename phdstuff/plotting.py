...
def plot_states(data_z, z_est, label, fname=None):
    if data_z is not None:
        titles = [f"True {label}", f"Estimated {label}"]
        states_list = [data_z[:1000], z_est[:1000]]
        ...
        ax = fig.gca()
        ax.imshow(z_est[None, :], aspect="auto")
        ax.set_yticks([])
        ax.set_title(f"Estimated {label}")
        plt.tight_layout()
        if fname:
            plt.savefig(fname,bbox_inches='tight', dpi=300)
        plt.show()
...
def plot_compare_true_and_predicted(true, predicted, index, fname=None):
    plt.clf()
    plt.plot(true, label="true")
    plt.plot(predicted, label="predicted")
    plt.title(f'Emission at {index} dimension')
    plt.legend()
    if fname:
        plt.savefig(fname,bbox_inches='tight', dpi=300)
    plt.show()
...
def show_time_series(
    ...
    for i in range(inputs.shape[-1]):
        ax.scatter(
            np.arange(inputs.shape[0]), inputs[:, i], marker="x", color=colors[i], s=20
        )
        ax.plot(
...
def show_time_series_sample(fig_size, inputs, fig_title=None, ylim=None):
    ...
    for i in range(inputs.shape[-1]):
        ...
        ax.plot(np.arange(inputs.shape[0]), inputs[:, i], lw=1, color=colors[i])
    ax.set_xlabel("T")
    if ylim is not None:
        ax.set_ylim(ylim)
    return fig
...
def show_discrete_states(fig_size, discrete_states_lk, segmentation, fig_title=None):
    fig = plt.figure(figsize=fig_size)
    if fig_title:
        plt.title(fig_title)
    ...
    s_seq = np.squeeze(segmentation)
    turning_loc = np.concatenate(([0], np.where(np.diff(s_seq))[0] + 1, [s_seq.size]))
    ...
    for tl in turning_loc:
        ax.axvline(tl, color="k", linewidth=2.0, linestyle="-.")
    ax.set_ylim(-0.1, 1.1)
    return fig
