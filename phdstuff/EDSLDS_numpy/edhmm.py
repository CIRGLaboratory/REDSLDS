...
class EDHMM:
    ...
    def loglikelihood(self, Zs, Ys):
        ...
        for Z, Y in zip(Zs, Ys):
            for t in range(1, len(Z)):
                ...
                if i == j:
                    l += self.O.loglikelihood(j, y)
                else:
                    l += (
                            self.A.loglikelihood(i, j) +
                            self.D.loglikelihood(i, di) +
                            self.O.loglikelihood(j, y)
                    )
        return l

    def set_transition_loglikelihood(self):
        """
        ...
        log.info('forming transition loglikelihoods from d = 1 to %s' % right)
                    ...
                    for dj in range(1, right + 1):
                        if di == 1:
                            self.l[(i, j, di, dj)] = np.exp(
                                self.A.loglikelihood(i, j) + self.D.loglikelihood(j, dj)
                            )
    ...
    def slice_sample(self, Z, min_u=0):
        ...
        for t in range(1, len(Z)):
            ...
            try:
                ...
                #     tfd.Uniform(
                #         low=min_u,
                #         high=self._get_likelihood(i, j, di, dj)).sample().reshape(())
                # )
            except KeyError:
                raise
        return np.array(u)
    ...
    def get_worthy(self, u, old_worthy):
        ...
        if len(worthy) == 0 and u > 0.:
            return self.get_worthy(-1, old_worthy)
    ...
    def beam_forward(self, Y, U, W=None):
        ...
        for i in self.states:
            for t, y in enumerate(Y):
                ol[i, t] = (self.O.loglikelihood(i, y))
            ...
            if t == 0:
                for i in self.states:
                    ...
                    for d in [1] + list(range(self.left[i], self.right[i] + 1)):
                        alphahat[t][i][d] = self.pi.loglikelihood((i, d))
    ...
    def beam(self, Y, min_u=0, its=100, burnin=50, name='beamer', online=True, sample_U=True, update_D=True,
        ...
        force_U : None or list of lists (None)
            you can force U to start off from a specific starting place if you like. If slds_experiment_1.py
        ...
        while not bored:
            ...
            if count > burnin:
                if count % 5 == 0:
                    ...
                    # pickle.dump(self.A.A, A_fh)
                    # pickle.dump(self.O.mu, O_m_fh)
                    # pickle.dump(self.O.tau, O_p_fh)
                    # pickle.dump(self.D.mu, D_m_fh)
                    # pickle.dump(Z_samples, Z_fh)
                    # pickle.dump(l, L_fh)
            # stop
            if count > its:
                bored = True
    ...
    def _get_loglikelihood(self, i, j, di, dj):
        if di == 1:
            return self.A.loglikelihood(i, j) + self.D.loglikelihood(j, dj)
        elif i == j and dj == di - 1:
            return 0
    ...
    def forward(self, Y):
        ...
        # build transition log loglikelihoods
        # l = {}
        # for i in self.states:
        ...
        #                 if di == 1:
        #                     l[(i, j, di, dj)] = self.A.loglikelihood(i, j) + self.D.loglikelihood(j, dj)
        #                 elif i == j and dj == di - 1:
        #                     l[(i, j, di, dj)] = 0
        #                 else:
                    ...
                    for di in range(min_d, max_d):
                        ...
                        for j in self.states:
                            for dj in range(min_d, max_d):
                                ...
                                alphahat[t][i][di] = elnsum(
                                    alphahat[t][i][di],
                                    self._get_loglikelihood(i, j, di, dj)
                                )
    ...
    def backward_sample(self, alphahat):
        ...
        for t in reversed(range(T - 1)):
            ...
            for j in worthy[Z[-1]]:
                try:
                    a[j[0]][j[1]] = alphahat[t][j[0]][j[1]] + self._get_loglikelihood(j[0], Z[-1][0], j[1], Z[-1][1])
                except KeyError:
                    a[j[0]][j[1]] = -10000000
            z = sample_z(a)
            Z.append(z)
