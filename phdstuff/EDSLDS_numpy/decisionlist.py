# Copied from https://github.com/slinderman/recurrent-slds/blob/master/rslds/decision_list.py
...
class DecisionList(object):
    ...
    Given an inputs x_n, the output label is generated
    from the model,
    ...
    def fit(self, X, y):
        ...
        for level in range(K-1):
            ...
            # sklearn binary logistic regression exposes coef_ as shape (1, D)
            # and intercept_ as shape (1,). Flatten explicitly before storing
            # into the decision-list parameter arrays.
            self.weights[level] = np.asarray(lrbest.coef_).reshape(-1)
            self.biases[level] = float(np.asarray(lrbest.intercept_).reshape(-1)[0])
